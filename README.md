...
`__CUSHION_WRAPPED__` blocks: they will be correctly unwrapped there. But beware that `__CUSHION_WRAPPED__` might be 
pasted several times, therefore if you use `#define`, you also need to use `#undef` for the same macro in order to avoid
duplication.
